...
def color_status(msg):
    if msg.rstrip().endswith("[OK]"):
        return msg.replace("[OK]", f"{GREEN}[OK]{RESET}")
    elif msg.rstrip().endswith("[FAILED]"):
        return msg.replace("[FAILED]", f"{RED}[FAILED]{RESET}")
    elif msg.rstrip().endswith("[SKIPPED]"):
        return msg.replace("[SKIPPED]", f"{BLUE}[SKIPPED]{RESET}")
    else:
        return msg


def log(msg, verbose=False):
    msg_col = color_status(msg)
    print(msg_col)
...
def init_node(node_num, verbose=False):
    ...
    if not Path(f"{data_dir}/PG_VERSION").exists():
        if not run([f"{BIN_DIR}/initdb", "-D", data_dir], verbose=verbose):
            log(msg + "[FAILED]", verbose)
            return
    if run([f"{BIN_DIR}/pg_ctl", "-D", data_dir, "-o", f"-p {port}",
            "-l", f"{data_dir}/server.log", "start"], verbose=verbose):
        log(msg + "[OK]", verbose)
    else:
        log(msg + "[FAILED]", verbose)
...
def cleanup_node(node_num, verbose=False):
    ...
    cmds = [
        ...
        [f"{BIN_DIR}/psql", "pgedge", f"-p{port}", "-c",
         "create extension if not exists spock"],
        [f"{BIN_DIR}/psql", "pgedge", f"-p{port}", "-c",
         "create extension if not exists dblink"],
        [f"{BIN_DIR}/psql", "pgedge", f"-p{port}", "-t", "-A", "-c",
         "SELECT 'SELECT pg_drop_replication_slot(' || quote_literal(slot_name) "
         "|| ');' FROM pg_replication_slots WHERE slot_type = 'logical';"],
        [f"{BIN_DIR}/psql", "-d", "pgedge", f"-p{port}", "-Atc",
         "SELECT 'SELECT pg_replication_origin_drop(' || quote_literal(roname) "
         "|| ');' FROM pg_replication_origin;"],
        [f"{BIN_DIR}/psql", "pgedge", f"-p{port}", "-c",
         "drop extension if exists spock; create extension spock"]
    ]
    ok = True
    for cmd in cmds:
        if ("pg_drop_replication_slot" in " ".join(cmd) or
                "pg_replication_origin_drop" in " ".join(cmd)):
            try:
                sql_out = subprocess.check_output(cmd, text=True,
                                                  stderr=subprocess.STDOUT)
                if sql_out.strip():
                    if not run([f"{BIN_DIR}/psql", "pgedge", f"-p{port}"],
                               input=sql_out, text=True, verbose=verbose):
                        ok = False
...
def main():
    parser = argparse.ArgumentParser(
        description="Spock PostgreSQL cluster manager (Python version)"
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("-i", "--init", action="store_true",
                       help="Initialize and start nodes")
    ...
    group.add_argument("-c", "--cleanup", action="store_true",
                       help="Cleanup databases/extensions on nodes")
    group.add_argument("-u", "--update-conf", action="store_true",
                       help="Update postgresql.auto.conf for nodes")
    group.add_argument("-a", "--all", action="store_true",
                       help="Full cycle: stop, cleanup, destroy, init, conf")
    parser.add_argument("-n", "--num-nodes", type=int,
                        default=DEFAULT_NUM_NODES,
                        help="Number of nodes (default: 3)")
    parser.add_argument("-v", "--verbose", action="store_true",
                        help="Show output to console as well as log file")
