...
BEGIN
    ...
    -- ============================================================================
    -- Step 3: Create replication slots on all source nodes for the new node
    -- ============================================================================
    FOR rec IN
        SELECT * FROM get_spock_nodes(src_dsn)
    LOOP
        ...
        RAISE LOG E'
        [STEP 3] Node: %, Location: %, Country: %, DSN: %
        ...
        slot_name := left(
            'spk_' || dbname || '_' || rec.node_name || '_sub_' ||
            rec.node_name || '_' || new_node_name, 64
        );
    ...
    -- ============================================================================
    -- Step 4: Trigger sync events on all source nodes except the source node
    -- ============================================================================
    FOR rec IN
        SELECT * FROM get_spock_nodes(src_dsn)
    LOOP
    ...
    -- ============================================================================
    -- Step 5: Create subscription from new node to source node and enable it
    -- ============================================================================
    PERFORM create_sub(
        new_node_dsn,
    ...
    -- ============================================================================
    -- Step 6: Advance replication slots on all source nodes except the source node
    -- ============================================================================
    FOR rec IN
        SELECT * FROM get_spock_nodes(src_dsn)
    LOOP
        IF rec.node_name != src_node_name THEN
            SELECT get_commit_timestamp(
                new_node_dsn,
                src_node_name,
                rec.node_name
            ) INTO sync_timestamp;

            slot_name := 'spk_' || dbname || '_' || src_node_name ||
                         '_sub_' || rec.node_name || '_' || new_node_name;

            PERFORM advance_replication_slot(
                rec.dsn,
                slot_name,
                sync_timestamp
            );
    ...
    -- ============================================================================
    -- Step 7: Create subscriptions from all source nodes to the new node and enable them
    -- ============================================================================
    FOR rec IN
        SELECT * FROM get_spock_nodes(src_dsn)
    LOOP
    ...
    -- ============================================================================
    -- Step 8: Enable subscriptions on the new node for all source nodes except itself
    -- ============================================================================
    FOR rec IN
        SELECT * FROM get_spock_nodes(src_dsn)
    LOOP
